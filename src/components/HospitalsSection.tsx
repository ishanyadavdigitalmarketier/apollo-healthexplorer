...
//             <Link to="/hospitals">View All Partner Hospitals</Link>
//           </Button>
//         </div>
//       </div>
//     </section>
...
// HospitalsSection.tsx - Enhanced responsiveness for all devices
import { Link } from 'react-router-dom';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardHeader } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
    ...
    <section className="py-12 sm:py-16 lg:py-20 bg-muted/30">
      <div className="container mx-auto px-4 sm:px-6">
        <div className="text-center mb-8 sm:mb-12 lg:mb-16">
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold mb-4">Top-Rated Partner Hospitals</h2>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 sm:gap-6 mb-12 sm:mb-16">
          {countries.map((country, index) => (
            <div key={index} className="text-center group cursor-pointer">
              <div className="bg-white rounded-lg p-3 sm:p-4 lg:p-6 shadow-md hover:shadow-lg transition-shadow border">
                <div className="text-3xl sm:text-4xl mb-2 sm:mb-3">{country.flag}</div>
                <h3 className="font-semibold text-base sm:text-lg mb-1">{country.name}</h3>
                <p className="text-xs sm:text-sm text-muted-foreground">{country.hospitals}</p>
              </div>
            </div>
          ))}
        </div>
                ...
                <div className="flex-1 p-4 sm:p-6">
                  <div className="flex justify-between items-start mb-2 sm:mb-3">
                    <div className="min-w-0">
                      <h3 className="text-lg sm:text-xl font-bold mb-1 line-clamp-1">{hospital.name}</h3>
                  ...
                  <div className="flex items-center gap-2 mb-2 sm:mb-3">
                    ...
                    <span className="text-xs sm:text-sm text-muted-foreground line-clamp-1">{hospital.location}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-2 sm:gap-4 mb-3 sm:mb-4">
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-xs text-muted-foreground hidden sm:inline">({hospital.reviews.toLocaleString()} reviews)</span>
                    </div>
                  </div>

                  <div className="mb-3 sm:mb-4">
                    <p className="text-xs sm:text-sm text-muted-foreground mb-2">Specialties:</p>
                  ...
                  <div className="space-y-1 mb-3 sm:mb-4 max-h-16 overflow-y-auto">
                    {hospital.features.map((feature, idx) => (
                      <div key={idx} className="flex items-center gap-2 text-xs">
                        <CheckCircle className="w-3 h-3 text-accent flex-shrink-0" />
                  ...
                  <div className="flex gap-2">
                    <Button asChild size="sm" className="flex-1 bg-accent hover:bg-accent/90 text-xs sm:text-sm">
                      <Link to="/book">Get Quote</Link>
                    </Button>
                  </div>
                </div>
