...
// TestimonialSection.tsx - Enhanced responsiveness for mobile and all devices
import { Card, CardContent } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Star, Quote } from 'lucide-react';
import { Link } from 'react-router-dom';
    ...
    <section className="py-12 sm:py-16 md:py-20 bg-background">
      <div className="container mx-auto px-4 sm:px-6">
        <div className="text-center mb-12 sm:mb-16">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-4">Real Patient Success Stories</h2>
        ...
        <div className="max-w-4xl mx-auto">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 sm:gap-6">
                    <div className="flex items-center gap-3 sm:gap-4">
                      <div className="w-10 h-10 sm:w-12 sm:h-12 bg-primary text-primary-foreground rounded-full flex items-center justify-center font-bold text-sm sm:text-base">
                        {testimonial.avatar}
                      </div>
                      <div>
                        <h4 className="font-semibold text-base sm:text-lg">{testimonial.name}</h4>
                        <p className="text-sm sm:text-base text-muted-foreground">{testimonial.country}</p>
                      </div>
                    </div>

                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 text-center w-full md:w-auto">
                      <div>
                        <p className="text-xs sm:text-sm text-muted-foreground">Treatment</p>
                        <p className="font-semibold text-xs sm:text-sm">{testimonial.treatment}</p>
                      </div>
                      <div>
                        <p className="text-xs sm:text-sm text-muted-foreground">Hospital</p>
                        <p className="font-semibold text-xs sm:text-sm">{testimonial.hospital}</p>
                      </div>
                      <div>
            ...
            <CardContent className="p-4 sm:p-6">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3 sm:gap-4 flex-1">
                  <div className="w-8 h-8 sm:w-10 sm:h-10 bg-primary-foreground text-primary rounded-full flex items-center justify-center font-bold text-xs sm:text-sm">
                    {testimonial.avatar}
                  </div>
                  <div>
                    <p className="font-semibold text-sm sm:text-base">{testimonial.treatment}</p>
                    <p className="text-xs sm:text-sm opacity-90">{testimonial.hospital}</p>
                  </div>
                </div>
                <div className="text-right">
          ...
          <div className="text-center">
            <Button asChild className="bg-accent hover:bg-accent/90 text-sm sm:text-base px-6 sm:px-8">
              <Link to="/story">Share Your Success Story</Link>
            </Button>
          </div>
        </div>
