...
// HeroSection.tsx - Enhanced responsiveness for mobile and all devices
import { useState } from 'react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
  ...
  const renderTabContent = () => {
    ...
    switch (activeTab) {
      case 'treatments':
        ...
        return (
          <div className="space-y-4">
            <h3 className="text-lg sm:text-xl font-semibold flex items-center gap-2 justify-center sm:justify-start">
              ...
              {(query ? filteredTreatments : currentData.treatments).map((treatment, index) => (
                <div key={index} className="border rounded-lg p-3 sm:p-4 bg-card">
                  ...
                  <div className="flex flex-col sm:flex-row justify-between text-xs sm:text-sm text-muted-foreground mt-2 gap-1 sm:gap-0">
                    ...
                    <span className="flex items-center gap-1"><DollarSign size={12} className="sm:size-14" /> {treatment.cost}</span>
                  </div>
                  {/* <Button variant="outline" size="sm" className="mt-3 w-full">Book Now</Button> */}
                    <Button variant="outline" size="sm" className="mt-3 w-full">
                    <Link to="/book">Book Now</Link>
            ...
            <div className="space-y-3">
              {(query ? filteredDoctors : currentData.doctors).map((doctor, index) => (
                <div key={index} className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 p-3 sm:p-4 border rounded-lg bg-card">
                  ...
                  <Button variant="outline" size="sm" asChild className="ml-auto sm:ml-0 w-full sm:w-auto mt-2 sm:mt-0">
                    <Link to="/book">Book Appointment</Link>
                  </Button>
                </div>
              ))}
      ...
      <div className="relative container mx-auto px-4 py-12 sm:py-20">
        <div className="max-w-4xl w-full">
          ...
          <div className="text-white mb-8 sm:mb-12">
            ...
            <p className="text-base sm:text-lg md:text-xl mb-6 sm:mb-8 opacity-90 max-w-2xl leading-relaxed">
              Connect with top hospitals and specialists worldwide. Quality medical care at affordable prices, with personalized support every step of the way.
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl p-4 sm:p-6 md:p-8 shadow-2xl">
            <div className="mb-4 sm:mb-6">
              ...
              <p className="text-sm sm:text-base text-muted-foreground">Search from thousands of treatments, hospitals, and specialists</p>
            </div>
            ...
            <Button 
              ...
              onClick={handleSearch}
            >
              <Search size={18} className="mr-2 sm:size-20" />
              Search
            </Button>
                ...
                <div className="flex flex-wrap gap-2 justify-center sm:justify-start">
                  {['Heart Surgery', 'Orthopedic Surgery', 'Cosmetic Surgery', 'Dental Treatments', 'Eye Surgery', 'Cancer Treatment'].map((treatment, index) => (
                    <Badge 
                      key={index} 
                      variant="secondary" 
                      className="cursor-pointer text-xs sm:text-sm hover:bg-primary hover:text-primary-foreground transition-colors px-2 py-1"
                    >
                      {treatment}
                    </Badge>
                  ))}
